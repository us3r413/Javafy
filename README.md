# Music Chat Application

A Java-based chat application with music sharing and SVG image conversion features.

## Features

- Real-time chat between host and clients
- Send and receive MP3 music files
- Switch between Host and Client modes
- SVG to image conversion utility (using Apache Batik)
- Modern Swing-based UI

## Technologies

- Java
- Swing (UI)
- Apache Batik (SVG conversion)

## Getting Started
Method 1:
1. Clone the repository:
   https://github.com/us3r413/Javafy
2. Open the project in IntelliJ IDEA.
3. Make sure you have Java 8 or above installed.
4. Build (Javafy.java) and run the project.
Method 2:
1. Download the compiled JAR file from:
   https://drive.google.com/drive/folders/12jRv_Otr0hk2tWW7PITDa4w6f4JT7i8K?usp=sharing
2. Run the bat file to run the application
3. Enjoy!
## Usage

- Start the application.
- Choose Host or Client mode.
- Enter your name and (if client) the host IP.
- Chat and share MP3 files with other users.
- Use the SVG to image converter utility as needed.

## Dependencies

- JavaFX
- Apache Batik
- Java Swing

## License

This project is licensed under the MIT License.

## Author

us3r413, SeanShiue2005
